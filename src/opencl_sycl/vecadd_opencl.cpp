...
double get_event_time(const Event &event) {
  return ((double)event.getProfilingInfo<CL_PROFILING_COMMAND_END>() -
          (double)event.getProfilingInfo<CL_PROFILING_COMMAND_START>()) /
         1000000.0;
}

#define EVENT_TIME(X) \
  printf("Event  %s: take %g ms.\n", (#X), get_event_time(X))
...
int main(int argc, char *argv[]) {
  ...
  cout << "Time taken to process " << DIM << " elements  is : " << time_taken
       << " ms" << endl;
...
double vect_add_opencl(int DIM, const int *a, const int *b, int *c) {
  ...
  cout << "Selected Platform is  : "
       << selectedPlatform.getInfo<CL_PLATFORM_NAME>() << endl;
  ...
  // copy A
  Event copyA;
  queue.enqueueWriteBuffer(aBuffer, CL_TRUE, 0, sizeof(int) * DIM, a, nullptr,
                           &copyA);

  // copy B
  Event copyB;
  queue.enqueueWriteBuffer(bBuffer, CL_TRUE, 0, sizeof(int) * DIM, b, nullptr,
                           &copyB);

  // execution of kernel
  Event executeKernel;
  queue.enqueueNDRangeKernel(kernel, NullRange, NDRange(DIM), NullRange,
                             nullptr, &executeKernel);

  // read results
  Event readC;
  queue.enqueueReadBuffer(cBuffer, CL_TRUE, 0, sizeof(int) * DIM, c, nullptr,
                          &readC);

  queue.finish();
  queue.flush();
  auto totalTime = get_event_time(copyA) + get_event_time(copyB) +
                   get_event_time(executeKernel) + get_event_time(readC);
